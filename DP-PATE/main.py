...
if __name__ == '__main__':
    ...
    if chest:
        ...
        train_teacher_loader_c, train_student_loader_c, test_loader_c = load_data(train_teacher_dataset_c,
                                                                                  train_student_dataset_c,
                                                                                  test_dataset_c, c_args.batch_size)
        
        # Train teachers
        print("Training teacher")
        teacher = Teacher(c_args, CNN, c_args.num_classes, n_teachers=c_args.n_teachers, stdev=c_args.sigma)
        ...
        # use train dataset as the dataset where labels are ignored (differentially private)
        N = NoisyDataset(train_student_loader_c, teacher.predict)
        ...
        # Get epsilon
        c_pate_epsilon = get_epsilon_momentents_gaussian_dp(len(train_student_dataset_c), c_args.sigma,
                                                            c_args.student_epochs, c_args.batch_size)

        # Put metrics in txt file
        dump_metrics_to_json("dp_pate_chest_metrics.txt", c_pate_runtime, c_pate_memory, c_student_accuracy,
                             c_pate_epsilon, is_dp=True)
    ...
    if knee:
        ...
        train_teacher_loader_k, train_student_loader_k, test_loader_k = load_data(train_teacher_dataset_k,
                                                                                  train_student_dataset_k,
                                                                                  test_dataset_k, k_args.batch_size)

        # Train teachers
        print("Training teacher")
        teacher = Teacher(k_args, CNN, k_args.num_classes, n_teachers=k_args.n_teachers, stdev=k_args.sigma)
        teacher.train(train_teacher_loader_k)  # use valid dataset as a partition for teachers

        # Compile dataset from teachers
        print("Creating Noisy Dataset")
        # use train dataset as the dataset where labels are ignored (differentially private)
        N = NoisyDataset(train_student_loader_k, teacher.predict)
        ...
        # Get epsilon
        k_pate_epsilon = get_epsilon_momentents_gaussian_dp(len(train_student_dataset_k),
                                                            k_args.sigma, k_args.student_epochs, k_args.batch_size)
